...
def preproc(img, input_size, swap=(2, 0, 1)):
    '''加掩码，防止检测到指定pen之外的猪'''
    img2 = img.copy()
    mask = cv2.imread(r"D:\zlw\mask PBVD.jpg", cv2.IMREAD_GRAYSCALE)
    mask = mask / 255.0
    ...
    padded_img2[: int(img.shape[0] * r), : int(img.shape[1] * r)] = resized_img2
    padded_img[: int(img.shape[0] * r), : int(img.shape[1] * r)] = resized_img

    padded_img = padded_img.transpose(swap)
    padded_img = np.ascontiguousarray(padded_img, dtype=np.float32)

    padded_img2 = padded_img2.transpose(swap)
    padded_img2 = np.ascontiguousarray(padded_img2, dtype=np.float32)
    return padded_img2, padded_img, r
