...
class KalmanFilterNew(object):
    ...
    **Example**
    ...
    you are trying to solve. 切记维度一致
    Parameters
    ----------
    dim_x : int
    ...
    def __init__(self, dim_x, dim_z, dim_u=0):
        ...
        self.x = zeros((dim_x, 1))        # state 状态
        self.P = eye(dim_x)               # uncertainty covariance x的协方差
        self.Q = eye(dim_x)               # process uncertainty 误差协方差
        self.B = None                     # control transition matrix 控制u前的系数
        self.F = eye(dim_x)               # state transition matrix 状态转移矩阵
        self.H = zeros((dim_z, dim_x))    # measurement function 测量转移矩阵
        self.R = eye(dim_z)               # measurement uncertainty 观测噪声协方差
        self._alpha_sq = 1.               # fading memory control
        self.M = np.zeros((dim_x, dim_z)) # process-measurement cross correlation 互相关
        ...
        self.y = zeros((dim_z, 1))        # 测量
        self.S = np.zeros((dim_z, dim_z)) # system uncertainty 系统不确定性
        self.SI = np.zeros((dim_z, dim_z)) # inverse system uncertainty 逆系统不确定性

        # identity matrix. Do not alter this.
        self._I = np.eye(dim_x)

        # these will always be a copy of x,P after predict() is called 用于predict
        self.x_prior = self.x.copy()
        self.P_prior = self.P.copy()

        # these will always be a copy of x,P after update() is called 用于update
