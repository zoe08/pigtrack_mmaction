...
from tracker.deepsort_tracker import kalman_filter, linear_assignment, iou_matching
...
class Tracker:
    ...
    def update(self, detections, classes):
        ...
        # if redet:
        #     # 对于未匹配成功的detection，初始化为新的track，赋予新的id，这里可以加阈值，不去赋予新值
        #     if frame_idx <= 2:
        #         for detection_idx in unmatched_detections:
        #             self._initiate_track(detections[detection_idx],
        #                                  classes[detection_idx].item())  # 视频前两帧之后，就不希望有新的track
        #     else:  # 对udet重新做一次IOU匹配，使跟踪更加稳定
        #         for detection_idx in unmatched_detections:
        #             for track_idx in unmatched_tracks:
        #                 if self.tracks[track_idx].mean[0:3] == [0, 0, 0, 0]:
        #                     pass
        #                 else:
        #                     IOU = iou_matching.bbox_iou(detections[detection_idx].tlwh,
        #                                                 self.tracks[track_idx].to_tlwh())
        #                     if IOU >= 0.5:
        #                         matches.append((track_idx, detection_idx))
        #                         track_indices = list(range(len(self.tracks)))
        #                         unmatched_tracks = list(set(track_indices) - set(k for k, _ in matches))
        #                         unmatched_detections = unmatched_detections.remove(detection_idx)
...
class DeepSort(object):
    ...
    def update(self, output_results, scale, ori_img):
        ...
        for track in self.tracker.tracks:
            ...
            score = 1  # 'Track' object has no attribute 'confidence'
            outputs.append(np.concatenate(([x1, y1, x2, y2], [track_id], [score], [class_id])).reshape(1, -1))
        if len(outputs) > 0:
